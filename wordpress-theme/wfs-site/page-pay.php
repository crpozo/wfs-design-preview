...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Alfa+Slab+One&family=Montserrat:wght@400;500;600;700&display=swap">
<style>
...
.dato__val a { color: inherit; text-decoration: underline; text-underline-offset: 3px; }
.dato__val {
...
var CUENTA_PC = {
  ...
  banco:    'Bank of America, Business Checking',
  direccion:'1145 Enterprise Drive, Port Charlotte, FL 33953',
  cuenta:   '898162811307',
  rutaWire: '026009593',
  rutaACH:  '063100277'
};
...
function opciones(sede) {
  ...
  return [
    ...
    { id: 'cashapp', nombre: 'Cash App', sub: sede.cashtag, url: sede.cashapp },
...
/* ── pintado ─────────────────────────────────────────────────────────────── */
/* Un correo se pinta como enlace mailto: ademas de copiarlo, se puede tocar. */
function valor(v) {
  v = String(v);
  return /^[^@\s]+@[^@\s]+\.[a-z]+$/i.test(v) ? '<a href="mailto:' + esc(v) + '">' + esc(v) + '</a>' : esc(v);
}
  ...
  cont.innerHTML = opciones(sede).map(function (o) {
    ...
    var datos = (o.panel.datos || []).map(function (d) {
      return '<div class="dato"><div class="dato__eti">' + esc(d[0]) + '</div>' +
        '<div class="dato__fila"><span class="dato__val">' + valor(d[1]) + '</span>' +
