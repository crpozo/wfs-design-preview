  ...
  <section style={{ background: 'var(--white)', padding: 'clamp(56px, 8vh, 90px) 0' }}>
    <div className="container">
      ...
      <div style={{
        ...
        paddingBottom: 24, marginBottom: 36, borderBottom: '2px solid var(--ink)',
      }}>
        <h2 className="display" style={{
          margin: 0, fontSize: 'clamp(26px, 2.8vw, 40px)', lineHeight: 1.02,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {CONTACT_YARDS.map((y) => (
          <article key={y.key} style={{
            border: '1px solid rgba(0,16,17,0.12)', borderTop: '3px solid var(--tangerine)',
            padding: '28px 30px 30px',
            display: 'flex', flexDirection: 'column', gap: 20,
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div>
                <h3 className="display" style={{ margin: '0 0 6px', fontSize: 27, lineHeight: 1 }}>{y.name}</h3>
                <p style={{ margin: 0, fontSize: 14.5, color: 'var(--charcoal)' }}>{y.address}</p>
              </div>
              <span className="mono" style={{
                flexShrink: 0,
                background: 'var(--ink)', color: 'var(--white)',
                padding: '5px 10px', fontSize: 11.5, letterSpacing: '0.18em', fontWeight: 700,
            ...
            <div style={{ display: 'grid', gap: 12, paddingTop: 4 }}>
              <a href={y.tel} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span aria-hidden style={{
                  width: 34, height: 34, flexShrink: 0, border: '1.5px solid var(--ink)',
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tangerine)',
                }}>
                ...
                <span className="display" style={{ fontSize: 21, fontWeight: 800, letterSpacing: '0.01em', color: 'var(--ink)' }}>{y.phone}</span>
              </a>
              <a href={`mailto:${y.email}`} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ fontSize: 14, color: 'var(--laser-blue)', wordBreak: 'break-all' }}>{y.email}</span>
              </a>
            </div>
...
const ContactForm = () => {
  ...
  const expectations = [
    { EN: 'Replies during business hours, usually the same day', ES: 'Respondemos en horario laboral, normalmente el mismo día' },
    ...
    <div className="container">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 0, alignItems: 'stretch' }}>
        ...
        <div className="wfs-brand-texture" style={{
          backgroundColor: 'var(--ink)', color: 'var(--white)',
          padding: 'clamp(32px, 4vw, 52px)',
          display: 'flex', flexDirection: 'column', gap: 22,
        }}>
          <h2 className="display" style={{
            margin: 0, fontSize: 'clamp(26px, 2.6vw, 38px)', lineHeight: 1.05,
            textTransform: 'uppercase', fontWeight: 800, color: 'var(--white)',
          }}>
            {t('Send us a', 'Envíanos un')}{' '}
            <span style={{ color: 'var(--tangerine)' }}>{t('message.', 'mensaje.')}</span>
          </h2>
          <p style={{ margin: 0, fontSize: 14.5, lineHeight: 1.65, color: 'var(--alice-blue)', maxWidth: 340 }}>
          ...
          <ul style={{ listStyle: 'none', margin: 'auto 0 0', padding: 0, display: 'grid', gap: 12 }}>
            {expectations.map((x, i) => (
              <li key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', fontSize: 13.5, lineHeight: 1.5, color: 'var(--alice-blue)' }}>
          ...
          {submitted ? (
            <div style={{ padding: '48px 0', textAlign: 'center' }}>
              <span aria-hidden style={{
                ...
                width: 60, height: 60, borderRadius: '50%', background: 'var(--tangerine)',
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} style={{ display: 'grid', gap: 18 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12.5, color: 'var(--charcoal)', maxWidth: 300 }}>
