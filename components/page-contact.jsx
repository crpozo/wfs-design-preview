...
const CONTACT_YARDS = [
  {
    key: 'fm', name: 'Fort Myers', chip: { EN: 'HQ · FORT MYERS', ES: 'SEDE · FORT MYERS' }, address: '2621 Fowler St, Fort Myers, FL 33901', phone: '(239) 465-2482', tel: 'tel:2394652482', email: 'sales@westernfencesupply.com',
    ...
    key: 'pc', name: 'Port Charlotte', chip: { EN: 'PORT CHARLOTTE', ES: 'PORT CHARLOTTE' }, address: '1145 Enterprise Dr, Port Charlotte, FL 33953', phone: '(941) 467-5821', tel: 'tel:9414675821', email: 'salespc@westernfencesupply.com',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {CONTACT_YARDS.map((y) => (
          <article key={y.key} style={{
            border: '1px solid rgba(38, 49, 102,0.12)', borderTop: '3px solid var(--tangerine)', padding: '28px 30px 30px', display: 'flex', flexDirection: 'column', gap: 20, }}>
...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError((err && err.message) || t(
        "We couldn't send your message. Please call (239) 465-2482 or email sales@westernfencesupply.com.", 'No pudimos enviar tu mensaje. Llama al (239) 465-2482 o escribe a sales@westernfencesupply.com.'
      ));
    } finally {
      busy.current = false;
      setSending(false);
  ...
  const inputStyle = {
    width: '100%', padding: '14px 16px', border: '1px solid rgba(38, 49, 102,0.25)', background: 'var(--white)', fontFamily: 'var(--sans)', fontSize: 15.5, color: 'var(--ink)', };
  const labelStyle = {
  ...
  <section id="message" style={{ background: 'var(--white)', padding: '0 0 clamp(64px, 9vh, 110px)', scrollMarginTop: 120 }}>
    <div className="container">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 0, alignItems: 'stretch' }}>
        ...
        {/* Form card */}
        <div style={{
          background: 'var(--white)', border: '1px solid rgba(38, 49, 102,0.15)', padding: 'clamp(28px, 3.5vw, 48px)', }}>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 18 }}>
              ...
              {error && (
                <p role="alert" style={{
                  margin: 0, padding: '14px 16px', borderRadius: 12, background: 'rgba(233, 106, 44,0.08)', border: '1px solid rgba(233, 106, 44,0.35)', fontSize: 15.5, lineHeight: 1.5, color: 'var(--ink)', }}>{error}</p>
              )}
            </form>
          )}
        </div>
