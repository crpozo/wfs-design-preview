...
const ResourceArticles = () => {
  const t = useT();
  const posts = [
    { tag: { EN: 'Materials', ES: 'Materiales' }, title: { EN: 'Top 4 Durable Fence Materials for SW Florida', ES: 'Los 4 materiales de cerca más duraderos para el suroeste de Florida' }, read: { EN: '6 min', ES: '6 min' }, img: 'aluminum' }, { tag: { EN: 'How-to', ES: 'Cómo hacerlo' }, title: { EN: 'How to Measure for Fence Material in Florida', ES: 'Cómo medir el material de cerca en Florida' }, read: { EN: '4 min', ES: '4 min' }, img: 'chainlink' }, { tag: { EN: 'Pricing', ES: 'Precios' }, title: { EN: 'How Much Does a Fence Cost in Florida in 2026?', ES: '¿Cuánto cuesta una cerca en Florida en 2026?' }, read: { EN: '8 min', ES: '8 min' }, img: 'vinyl' }, { tag: { EN: 'Code', ES: 'Código' }, title: { EN: 'Florida Pool Code: Enclosure Requirements Explained', ES: 'Código de piscinas de Florida: requisitos de cerramiento explicados' }, read: { EN: '7 min', ES: '7 min' }, img: 'metal' }, { tag: { EN: 'Comparison', ES: 'Comparativa' }, title: { EN: 'Aluminum vs Vinyl vs Chain Link, Picking a System', ES: 'Aluminio vs vinilo vs malla ciclónica: cómo elegir un sistema' }, read: { EN: '9 min', ES: '9 min' }, img: 'aluminum' }, { tag: { EN: 'HVHZ', ES: 'HVHZ' }, title: { EN: 'Hurricane Zone Fencing: EC Fence vs DuraFence', ES: 'Cercas para zona de huracanes: EC Fence vs DuraFence' }, read: { EN: '5 min', ES: '5 min' }, img: 'ecfence' }, ];
  return (
            ...
            <article key={i} style={{
              ...
              <div style={{ padding: 24, flex: 1, display: 'flex', flexDirection: 'column' }}>
                <h3 className="display" style={{
                  margin: '0 0 14px', fontSize: 21.5, lineHeight: 1.15, }}>{t(p.title)}</h3>
                <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span className="mono" style={{
                    fontSize: 13.5, letterSpacing: '0.18em', color: 'var(--charcoal)', textTransform: 'uppercase', }}>{t(p.read)} {t('read', 'de lectura')}</span>
                  <span style={{
                    fontSize: 14.5, color: 'var(--laser-blue)', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6, }}>{t('Read', 'Leer')} <ArrowRight size={12}/></span>
                </div>
              </div>
            </article>
          ))}
  ...
  <section style={{ background: 'var(--parchment)', padding: '120px 0' }}>
    <div className="container">
      ...
      <div style={{
        ...
        <div style={{
          display: 'grid', gridTemplateColumns: '1.2fr repeat(4, 1fr)', background: 'var(--ink)', color: 'var(--white)', }}>
          {[
            { label: '', img: null, href: null }, { label: t('Vinyl / PVC', 'Vinilo / PVC'), img: FENCE_IMG.vinyl, href: 'vinyl.html' }, { label: t('Aluminum', 'Aluminio'), img: FENCE_IMG.aluminum, href: 'aluminum.html' }, { label: t('Chain Link', 'Malla ciclónica'), img: FENCE_IMG.chainlink, href: 'chain-link.html' }, { label: t('Metal / DuraFence', 'Metal / DuraFence'), img: FENCE_IMG.metal, href: 'metal.html' }, ].map((h, i) => (
            h.img ? (
              <a key={i} href={h.href} style={{
                position: 'relative', display: 'block', overflow: 'hidden', minHeight: 'clamp(170px, 15vw, 210px)', textDecoration: 'none', color: 'var(--white)', borderLeft: '1px solid rgba(255,255,255,0.1)', }}
              ...
              <div key={i} style={{
                display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 'clamp(22px, 2vw, 30px)', minHeight: 'clamp(170px, 15vw, 210px)', }}>
                <div className="mono" style={{
                  fontSize: 12.5, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(219,233,238,0.55)', marginBottom: 12, }}>{t('At a glance', 'De un vistazo')}</div>
      ...
      <p style={{ marginTop: 14, fontSize: 14, lineHeight: 1.6, color: 'var(--charcoal)' }}>
        {useT()('Specs shown are general guidance. Wind ratings, warranties, availability and performance depend on the specific product, configuration and project, we confirm exact numbers on your quote.', 'Las especificaciones mostradas son una guía general. Las clasificaciones de viento, garantías, disponibilidad y desempeño dependen del producto, la configuración y el proyecto específicos; confirmamos los números exactos en tu cotización.')}
      </p>
    </div>
  </section>
  );
...
const PoolCodeSection = () => {
  ...
  return (
    <SplitBlock
      ...
      bullets={[
        t('4 ft minimum height with no horizontal handhold below 45"', 'Altura mínima de 4 pies sin apoyo horizontal para manos por debajo de 45"'), t('Pickets spaced ≤ 3" apart for pool code', 'Barrotes separados ≤ 3" para cumplir el código de piscinas'), t('Self-closing, self-latching gates ≥ 54" from grade', 'Portones de cierre y pestillo automáticos a ≥ 54" del nivel del suelo'), t('Manufacturer spec sheets available with your quote', 'Fichas técnicas del fabricante disponibles con tu cotización'), ]}
  ...
  <section style={{ background: 'var(--white)', padding: '120px 0' }}>
    <div className="container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {[
          [t('EC Fence, Exposure C Wind', 'EC Fence, viento de exposición C'), 'PDF · 1.2 MB'], [t('Aluminum 3/4/5 Rail Profiles', 'Perfiles de aluminio de 3/4/5 rieles'), 'PDF · 980 KB'], [t('DuraFence Aluminum Board', 'Tablero de aluminio DuraFence'), 'PDF · 1.4 MB'], [t('Catalyst Vinyl, Privacy + Picket', 'Vinilo Catalyst, privacidad + barrotes'), 'PDF · 1.1 MB'], [t('Chain Link Gauges & Mesh', 'Calibres y malla de malla ciclónica'), 'PDF · 620 KB'], [t('Florida Pool Code Profiles', 'Perfiles del código de piscinas de Florida'), 'PDF · 1.6 MB'], ].map(([t, meta], i) => (
          <a key={i} href="#" style={{
